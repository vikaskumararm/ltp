...
static char *carg, *cdst, *ciface, *ipv6_arg, *nexthop;
...
static void rtnl_route(int iface, char *cdst, uint32_t prefix, char *cgw, int type)
{
	...
	if (!inet_pton(family, cdst, &dst))
		tst_brk(TBROK, "inet_pton failed");
	...
	if (nl == NULL)
		tst_brk(TBROK, "mnl_socket_open failed");

	if (mnl_socket_bind(nl, 0, MNL_SOCKET_AUTOPID) < 0)
		tst_brk(TBROK, "mnl_socket_bind failed");

	portid = mnl_socket_get_portid(nl);

	if (mnl_socket_sendto(nl, nlh, nlh->nlmsg_len) < 0)
		tst_brk(TBROK, "mnl_socket_sendto failed");

	ret = mnl_socket_recvfrom(nl, buf, sizeof(buf));
	if (ret < 0)
		tst_brk(TBROK, "mnl_socket_recvfrom failed (%d)", ret);

	ret = mnl_cb_run(buf, ret, seq, portid, NULL, NULL);
	if (ret < 0)
		tst_brk(TBROK, "mnl_cb_run failed (%d)", ret);
...
static void run(void)
{
	...
	for (i = 0; i < num_loops; i++) {
		...
		rtnl_route(iface, cdst, prefix, NULL, RTM_NEWROUTE);
		rtnl_route(iface, cdst, prefix, NULL, RTM_DELROUTE);
	}
