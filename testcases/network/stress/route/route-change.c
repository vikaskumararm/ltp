...
static void rtnl_route(int iface, char *cdst, uint32_t prefix, char *cgw, int type)
{
	...
	if (cgw) {
		fprintf(stderr, "%s:%d %s(): using cgw: '%s'\n", __FILE__, __LINE__, __func__, cgw); // FIXME: debug
		if (is_ipv6)
...
static void send_udp(char *local, char *remote)
{
	...
	SAFE_CONNECT(fd, remote_addrinfo->ai_addr, remote_addrinfo->ai_addrlen);
	ssize_t len = SAFE_SEND(1, fd, msg, strlen(msg), MSG_CONFIRM);
...
{
	int i;
	char *cdst, *cgw = NULL, *local = NULL, *remote;

	tst_res(TINFO, "Adding and deleting route with different destination %d times", num_loops);
	for (i = 1; i <= num_loops; i++) {
		if (gw_arg) {
			cdst = tst_ipaddr_un(family, i, 0);
			local = tst_ipaddr_un(family, 1, 2);
			remote = tst_ipaddr_un(family, 1, 1);
			cgw = tst_ipaddr_un(family, 1, i + 2);
		} else {
			cdst = tst_ipaddr_un(family, i, 0);
			local = tst_ipaddr_un(family, i, 2);
			remote = tst_ipaddr_un(family, i, 1);
		}
		...
		rtnl_route(iface, cdst, prefix, cgw, RTM_DELROUTE); // FIXME: keep it
	}
