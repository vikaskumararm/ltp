...
static char *c_opt, *d_opt, *D_opt, *g_opt, *ipv6_opt, *l_opt, *r_opt;
static int family = AF_INET;
static int num_loops = 10000;
static int iface, is_ipv6, num_data;
...
static void setup(void)
{
	...
	if (!num_data)
		tst_brk(TBROK, "wrong -l parameter ('%s')", l_opt);

	data = SAFE_MALLOC(sizeof(struct ip) * num_data);

	ptr = strtok(l_opt, IP_ADDR_DELIMITER);
	while (ptr != NULL) {
		data[i].local = strdup(ptr);
		fprintf(stderr, "%s:%d %s(): data[%d].local: '%s'\n", __FILE__, __LINE__, __func__, i, data[i].local); // FIXME: debug
		ptr = strtok(NULL, IP_ADDR_DELIMITER);
		i++;
	}
	...
	fprintf(stderr, "%s:%d %s(): num_data: %d\n", __FILE__, __LINE__, __func__, num_data); // FIXME: debug
	if (num_data != num_data2)
		tst_brk(TBROK, "different count of IPs in -l and -r parameter (%d vs. %d)",
				num_data, num_data2);
	i = 0;
	ptr = strtok(r_opt, IP_ADDR_DELIMITER);
	while (ptr != NULL) {
		data[i].remote = strdup(ptr);
		fprintf(stderr, "%s:%d %s(): data[%d].remote: '%s'\n", __FILE__, __LINE__, __func__, i, data[i].remote); // FIXME: debug
		ptr = strtok(NULL, IP_ADDR_DELIMITER);
		i++;
	}
}
...
static struct tst_option options[] = {
	...
	{"d:", &d_opt, "-l x     Destination networks"},
	{"D:", &D_opt, "-d IFACE Interface to work on"},
	{"g", &g_opt, "-g       Change gateway instead of destination"},
	{"l:", &l_opt, "-l x     Local IPs"},
	{"r:", &r_opt, "-r x     Remote IPs"},
	{NULL, NULL, NULL}
};
