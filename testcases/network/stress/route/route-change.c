...
static char *carg, *ciface, *config_dir, *gw_arg, *ipv6_arg;
...
static void setup(void)
{
	...
	if (!config_dir)
		tst_brk(TBROK, "Missing server config directory, specify it with -B");

	SAFE_CHDIR(config_dir);
}
...
static void send_udp(int id, char *local, char *remote)
{
	...
	SAFE_ASPRINTF(&file, "%s/%d/port", cwd, id);
	SAFE_FILE_SCANF(file, "%d", &_port);
	...
	fprintf(stderr, "%s:%d %s(): send to, scanned port: %s (%d)\n", __FILE__, __LINE__, __func__, port, _port); // FIXME: debug
...
static struct tst_option options[] = {
	...
	{"B:", &config_dir, "-B x     Server config directory"},
