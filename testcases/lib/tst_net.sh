...
#
# OPTIONS
# -b: use also broadcast address (HOST_ID: IPv4: 255, IPv6: 0xffff), equivalent
# of -m 255 or -m 65535, cannot be combined with -f, -h, -l, -m, -n
#
...
tst_ipaddr_un()
{
	...
	while getopts "bc:fhl:m:np" opt; do
		case $opt in
			b)
				...
				#echo "TU b: set max_host_id: $max_host_id" # FIXME: debug
				b_arg=1
				;;
			c) counter="$OPTARG";;
			h)
			...
			f)
				...
				min_host_id=0
				max_host_id=$max_net_id
				#echo "TU f: set max_host_id: $max_host_id, min_host_id: $min_host_id" # FIXME: debug
				f_arg=1
				;;
			l)
	...
	if [ -z "$min_host_id" ]; then
		...
		#printf "EMPTY min_host_id, set to $min_host_id " # FIXME: debug
	fi

	if [ -z "$max_host_id" ]; then
		[ "$is_counter" ] && max_host_id=$((max_net_id - 1)) || max_host_id=$max_net_id
		#printf "EMPTY max_host_id, set to $max_host_id " # FIXME: debug
	fi
	...
	if [ "$is_counter" ]; then
		...
		#printf "TEST: host_id: $host_id, min_host_id: $min_host_id " # FIXME: debug
		if [ $host_id -lt 0 ]; then
		#if [ $host_id -eq -$min_host_id ]; then
			#echo "PEV: host_id: $host_id, -min_host_id: -$min_host_id" # FIXME: debug
			#host_id=$((max_host_id-min_host_id)) # FIXME: or -1 ?
	...
	#printf "min_host_id: $min_host_id, max_host_id: $max_host_id " # FIXME: debug
	net_id=$((net_id % (max_net_id + 1)))
	host_id=$((host_id % (max_host_id - min_host_id + 1) + min_host_id)) # orig, but also bad
	# FIXME: debug
	#echo "net_id: '$net_id', host_id: '$host_id', min_host_id: '$min_host_id', max_host_id: '$max_host_id'" >&2
	#if [ "$min_host_id" ]; then
